...
public:
  ...
  std::string BarakSpecialHello(const std::string &user) {
    // Data we are sending to the server.
    HelloRequest request;
    request.set_name(user);
    ...
    // The actual RPC.
    Status status = stub_->BarakSpecialHello(&context, request, &reply);
...
int main(int argc, char **argv) {
  ...
  std::string barak_reply = greeter.BarakSpecialHello("EVER !!");
  std::cout << "Greeter received: " << reply << std::endl;
  std::cout << "The Server send: " << barak_reply << std::endl;
